...
const verifyToken = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    req.user = decoded; // Attach user info to request object (id, userName)
...
app.post("/goals/:userName", verifyToken, async (req, res) => {
  try {
    ...
    const newGoal = new Goal({
      ...goalData,
      userName: userName, // Ensure userName from URL is used
      userId: req.user.id, // Assign the userId from the token
      calculatedAt: new Date().toISOString(), // Add a timestamp for when it was calculated/saved
    });
...
// PUT (Update) an existing goal for a user
app.put("/goals/:userName/:id", verifyToken, async (req, res) => {
  try {
    const { userName, id } = req.params;
    const updateData = req.body;
    ...
    goal.calculatedAt = new Date().toISOString(); // Update timestamp
...
app.delete("/goals/:userName/:id", verifyToken, async (req, res) => {
  try {
    const { userName, id } = req.params;
...
app.delete("/transactions/:username/:id", verifyToken, async (req, res) => {
  const { username, id } = req.params;
...
// Manually trigger interest calculation (for testing) - if needed to be protected, add verifyToken
// app.post("/calculate-interest", verifyToken, async (req, res) => {
//     try {
//         await updateDailyInterest();
//         res.json({ message: "Interest calculation triggered" });
//     } catch (err) {
//         res.status(500).json(err);
//     }
// });
